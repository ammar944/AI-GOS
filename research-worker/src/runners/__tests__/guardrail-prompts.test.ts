...
describe('offer runner guardrail', () => {
  it('OFFER_CURRENT_ACTIVITIES_GUARDRAIL contains the exact header line', () => {
    // Lock in the exact header line so a future refactor can't silently
    // move the guardrail into an unrelated section and still pass the test.
    expect(OFFER_CURRENT_ACTIVITIES_GUARDRAIL).toContain('CURRENT MARKETING ACTIVITIES (context for offer analysis):');
  });
  ...
  it('OFFER_CURRENT_ACTIVITIES_GUARDRAIL has a carve-out for the empty-field case', () => {
    // Assert on the full sentence, not just a fragment — common phrases collide.
    expect(OFFER_CURRENT_ACTIVITIES_GUARDRAIL).toContain('If the field is empty or absent, ignore this rule');
  });

  it('OFFER_SYSTEM_PROMPT actually includes the guardrail (not just exports it)', () => {
    expect(OFFER_SYSTEM_PROMPT).toContain('CURRENT MARKETING ACTIVITIES (context for offer analysis):');
