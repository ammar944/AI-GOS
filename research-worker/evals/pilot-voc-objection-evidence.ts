#!/usr/bin/env node
/**
 * Pilot eval for the Voice of Customer per-Section Artifact pipeline.
 *
 * Usage:
 *   cd research-worker && npm run eval:pilot:voc-objection-evidence
 *   cd research-worker && npm run eval:pilot:voc-objection-evidence -- --company "Fellow" --url "https://fellow.app"
 */
...
} from '../src/agents/subagents/schemas/voc-objection-evidence';
...
function buildMinimalContext(args: CliArgs): string {
  return [
    ...
    'reviews, and firecrawl against the URL above to gather the evidence you',
    'need for Voice of Customer & Objection Evidence. The runner will turn',
    'your evidence transcript into the typed Section 04 Artifact after the',
    'tool loop ends.',
  ].join('\n');
}
...
function buildSubsectionChecks(
  ...
  return [
    {
      name: 'painLanguage',
      present: !!artifact.painLanguage,
      proseLength: artifact.painLanguage.prose.length,
      proseLongEnough: artifact.painLanguage.prose.length > 100,
      cardCount: artifact.painLanguage.quotes.length,
      minimumCardCount: 10,
    },
    {
      name: 'objections',
      present: !!artifact.objections,
      proseLength: artifact.objections.prose.length,
      proseLongEnough: artifact.objections.prose.length > 100,
      cardCount: artifact.objections.items.length,
      minimumCardCount: 5,
    },
    {
      name: 'switchingStories',
      present: !!artifact.switchingStories,
      proseLength: artifact.switchingStories.prose.length,
      proseLongEnough: artifact.switchingStories.prose.length > 100,
      cardCount: artifact.switchingStories.stories.length,
      minimumCardCount: 3,
    },
    {
      name: 'decisionCriteria',
      present: !!artifact.decisionCriteria,
      proseLength: artifact.decisionCriteria.prose.length,
      proseLongEnough: artifact.decisionCriteria.prose.length > 100,
      cardCount: artifact.decisionCriteria.criteria.length,
      minimumCardCount: 5,
    },
    {
      name: 'successLanguage',
      present: !!artifact.successLanguage,
      proseLength: artifact.successLanguage.prose.length,
      proseLongEnough: artifact.successLanguage.prose.length > 100,
      cardCount: artifact.successLanguage.quotes.length,
      minimumCardCount: 5,
    },
  ];
}
...
async function main(): Promise<void> {
  ...
  const spec = POSITIONING_SECTION_SPECS.positioningVoiceOfCustomer;
  const context = buildMinimalContext(args);

  console.log('=== Voice of Customer Pilot Eval ===');
  ...
  const parsed = VoiceOfCustomerArtifactSchema.safeParse(result.data);
  if (!parsed.success) {
    console.error('ERROR: result.data failed VoiceOfCustomerArtifactSchema.safeParse');
  ...
  const validation = validateVoiceOfCustomerMinimums(artifact);
