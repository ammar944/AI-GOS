...
import { researchIcpInputSchema } from "../schemas/input.ts";
import { researchIcpOutputSchema } from "../schemas/output.ts";
...
function validateOutput(path: string): void {
  ...
  const result = researchIcpOutputSchema.safeParse(raw);
  if (!result.success) {
...
function validateInput(path: string): void {
  const raw = readJson(path);
  const result = researchIcpInputSchema.safeParse(raw);
  if (!result.success) {
    process.stderr.write(
