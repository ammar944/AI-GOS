...
interface MediaPlanErrorProps {
  error: Error & { digest?: string }
  reset: () => void
}

export default function MediaPlanError({ error, reset }: MediaPlanErrorProps) {
  useEffect(() => {
    console.error("[MediaPlanError]", error)
  }, [error])
      ...
      <div className="max-w-md mx-auto px-6 flex flex-col items-center gap-6 text-center">
        ...
        <div className="flex flex-col gap-2">
          <h1 className="text-2xl font-semibold font-heading text-[var(--text-primary)]">
            Error Loading Media Plan
          </h1>
          <p className="text-[var(--text-secondary)] text-sm leading-relaxed">
            We couldn&apos;t load this media plan. It may have been deleted or you may not have access.
          </p>
        </div>
