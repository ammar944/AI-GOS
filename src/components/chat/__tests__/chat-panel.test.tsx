...
vi.mock('radix-ui', () => {
  const Root = ({ children, open }: { children: ReactNode; open?: boolean; onOpenChange?: (o: boolean) => void }) => {
    if (!open) return null;
    return <div data-testid="sheet-root">{children}</div>;
  };

  const Portal = ({ children }: { children: ReactNode }) => <>{children}</>;
  const Overlay = ({ children, ...props }: HTMLAttributes<HTMLDivElement>) => <div {...props}>{children}</div>;

  const Content = ({ children, ...props }: HTMLAttributes<HTMLDivElement>) => (
    <div data-testid="sheet-content" {...props}>{children}</div>
  );

  const Close = ({ children, ...props }: ButtonHTMLAttributes<HTMLButtonElement>) => (
