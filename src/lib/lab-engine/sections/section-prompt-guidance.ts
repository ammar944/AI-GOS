...
export const buyerICPMinimumGuidance = [
  ...
  "- When you cannot name a specific person, author a `segmentLabel` instead (e.g. \"VP of Finance at mid-market SaaS companies\", \"Controllers at 200–1000-employee firms\"). A persona with a substantive `segmentLabel` and a live `sourceUrl` is fully valid and preferred over shipping nothing.",
  "- The `segmentLabel` text MUST appear verbatim on the cited `sourceUrl` page — it is strict-checked against the live page text. Only use words and phrases you actually found on the fetched page. A fabricated or inferred segment that is not on the page will be dropped by the verifier.",
  "- Do not invent named people. If no named buyer identity exists and no `segmentLabel` can be grounded on a live page, use `body.personaReality.blockGap` instead of padding persona rows.",
...
export const voiceOfCustomerMinimumGuidance = [
  ...
  "- `source` must be one of `g2`, `capterra`, `trustpilot`, `trustradius`, `reddit`, `hackernews`, `sales-call`, `support-thread`, `twitter`, `other`; `painIntensity` must be `high`, `medium`, or `low`.",
...
export const paidMediaPlanMinimumGuidance = [
  ...
  "- PaidMediaPlanSectionOutput numeric fields are `totalMonths` and `phaseCount`, plus optional machine-sortable money siblings `monthlyBudgetValue`, `dailySpendValue`, and `dailyBudgetValue`; emit those as numbers. Do NOT emit `staticCount`, `videoCount`, or `totalPerAudience` — the runner computes them from the creative framework and brief capacity and overwrites any model-emitted counts.",
  ...
  "- `body.creativeStrategy` carries `prose` only; the runner computes `staticCount`, `videoCount`, and `totalPerAudience` (model-emitted counts are ignored); formats must be native to the CHANNEL POLICY's primary platform.",
