...
function buildSectionMinimumGuidance(
  ...
  if (definition.sectionOutputSchemaName === "DemandIntentSectionOutput") {
    return [
      ...
      "- Put a falsifiable signal on every keyword row: call the `keyword_volume` tool (SpyFu) with your candidate keywords in ONE bulk call (up to 100) to get monthly search volume + CPC + difficulty, and populate `monthlyVolume`, optional display `cpc`, and optional machine-sortable numeric siblings `monthlyVolumeValue`, `cpcValue`, and `difficulty` from it.",
      ...
      "- Numeric siblings are tool-derived only: when `keyword_volume` returns data for a keyword, set `monthlyVolumeValue` to `searchVolume`, `cpcValue` to `cpc`, and `difficulty` to `difficulty` as nonnegative numbers. If the tool returns a gap or no row for that keyword, omit the numeric siblings and name the gap in the existing display strings/prose; do not invent sortable numbers.",
