// MCP Tool Wrapper: PageSpeed
// betaZodTool wrapping Google PageSpeed Insights API for use by Anthropic SDK sub-agents
...
export const pagespeedTool = betaZodTool({
  name: 'pagespeed',
  description:
    'Get PageSpeed performance score and Core Web Vitals metrics for a URL.',
  ...
  run: async ({ url }) => {
    try {
      ...
      const lighthouseResult = data.lighthouseResult as
        | {
