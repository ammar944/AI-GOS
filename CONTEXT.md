...
## Research quality vocabulary

These terms were resolved in the 2026-06-07 research-quality alignment grilling. They exist to stop the recurring conflation of "the gate is green" with "this is 9/10 strategist work."

- **Research Quality ("9/10")** — the product's headline quality bar. **Resolved:** 9/10 means *a strong model standing in for a senior GTM strategist (GPT-5.5 or Claude) judges the Audit as both accurate and insightful*: built faithfully from the **Corpus** and **Onboarding** inputs, free of hallucination, with real grounded metrics (e.g. sales cycle, ACV), and carrying non-obvious strategist reasoning. It is **not** defined as the deterministic gate passing. The gate is a *floor* ("honest enough to judge, safe enough to show"), never the grade.

- **Trust axis vs Insight axis** — the two independent dimensions of Research Quality. **Resolved:** **Trust** = accuracy, grounding, no fabrication, honest gap-labeling (what the deterministic verifier and gate measure). **Insight** = non-obvious, decision-grade strategist reasoning that survives the senior-marketer "I already knew that" sweep. A run can be high-Trust and low-Insight (accurate but generic) — that is the current failure mode. Both are required for 9/10.

- **Drafting model vs Review model vs Judge model** — three distinct model *roles*, historically conflated as "the model." **Resolved:** **Drafting model** writes each Section's first pass. **Review/Thinker model** fact-checks each Section and runs the cross-section strategist critic (the in-pipeline insight engine). **Judge model** is the external, post-hoc strong model that scores the finished Audit against the 9/10 bar and never mutates it. These are independently chosen; a weak Drafting model with a strong Review model is a deliberate cost/quality split, not an accident.

- **Annotated ResearchInput (ARI)** — the unified shared-context object for a run. **Resolved:** the run's research context is **one** object (the existing `ResearchInput`) annotated with three things — **Provenance** on every fact, a **Coverage** map of required strategic inputs, and the cross-section critique — and threaded through every stage. It is authored **once** at the corpus→input merge (single writer); downstream stages read it; only the cross-section Thinker and each Section's own Artifact write (each a single writer). It is NOT a new database, graph, or orchestrator. This is the canonical alternative to scattered per-stage fixes.

- **Provenance** — where a fact came from. **Resolved:** every fact (onboarding field, corpus item, section claim) carries an origin label from the existing enum (`user-supplied | tool-measured | source-reported | model-estimated | unknown`). Load-bearing because the verifier must treat an operator-supplied number (e.g. their own ACV) as grounded, not hallucinated — the root cause of the TruthGate false-fail.

- **Coverage** — whether the run actually contains the strategic inputs a GTM decision needs (ACV, sales cycle, pricing, competitors, buyer titles, category, motion), judged by Provenance, not by counting sources. **Resolved:** Coverage replaces "count of URLs" as the corpus/research quality floor. A fact is `grounded`, `thin`, or `missing`. The deterministic gate and the offline Judge both read Coverage; each gap maps 1:1 to a Coverage item id, which is what makes the iterate loop actionable.

- **Evidence Ledger** — the typed claim store underlying ARI. **Resolved:** synonym for the claim layer of ARI (typed `{assertion, value, sourceUrl, confidence, tags, origin}` facts). Durable form is a run-scoped Supabase JSON record (never Markdown — models silently rewrite Markdown); in-flight transport into agents is AI SDK v6 `experimental_context`, never a module-level singleton (which would race under the parallel Section fan-out).
