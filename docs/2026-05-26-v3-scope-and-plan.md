...
> **⚠️ AMENDED 2026-05-26 — read with `docs/2026-05-26-v3-bg-execution-handoff.md` (the current execution spec). These deltas SUPERSEDE the text below:**
> 1. **Sections use REAL in-section tools — NOT corpus-only.** Decisions #2 and #9 and the `lab-section-job.ts:89` "Do NOT enable `LAB_ENGINE_LIVE_TOOLS`" anchor are **REVERSED** (ADR-0006). Flip the gate on with each section's registry allowlist; **Phase D** becomes "real-tool sections + delete the synthetic backfill in `corpus-to-research-input.ts` + relax the `ResearchInput` floors," not "enrich the corpus pass." The corpus stays the shared base only. (D2 adPresence + D3 visible-steps still apply.)
...
2. **Live research** — enrich + surface the SINGLE corpus pass; the 6 sections stay **corpus-only** (DeepSeek, ~30s each). No per-section live tools.
...
4. **Channel data** — media plan adds **one targeted client-channel research step** for sub-section #7 (client's own Google Ads/site/SEO/content); competitor `adPresence` added to Competitor research (G1); everything else synthesis. ("for now.")
...
Goal: real users hit the proven corpus-only DeepSeek path.
...
## PHASE D — Corpus enrichment (D2 + G1)
- **D1 [Codex+Claude]** Enrich the single corpus pass (targeted queries) so 6/6 pass `validateMinimums` on real URLs (the corpus-poverty fix). *Verify:* corpus-coverage eval — N real URLs → 6/6 pass.
...
- **D3 [Codex]** Surface corpus research as visible steps (query→search→synthesis) feeding Phase C. *Verify:* corpus phase shows steps in UI.
...
- **E2 [Codex]** Targeted client-channel research step for sub-section #7 (D4). *Verify:* #7 populated from real client-channel data.
...
- **F1 [Codex]** Remove `/research-v2` page, orchestrate `draft/deep/managed` branches, old `src/lib/media-plan/*` heavy pipeline + components + orphan tests. KEEP the worker. *Verify:* tsc 0, tests green, no dangling refs, build 0. **Depends:** A,B,C,D,E green.

## PHASE G — Prod (DeepSeek-only)
- **G1 [Claude]** Vercel production env (`DEEPSEEK_API_KEY` etc.), promote preview → prod. *Verify:* prod run completes 6/6 + media plan. Key rotation = tracked follow-up, non-blocking.

---

## Sequencing
A ships immediately (gated). B/C/D/E parallelizable across Codex tasks once A lands (B before C/E-render; D before E-synthesis quality). F after the new surfaces are proven. G last.
