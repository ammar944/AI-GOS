...
The survey found exactly this duplication. Each detailed `SKILL.md` carries a "Capability Gaps" block (~20 lines: tool-failure / rate-limit / evidence-gap handling) and the stub skills are ~44% boilerplate (`voice-of-customer`, `demand-intent`, `offer-diagnostic` are 45 lines each, ~20 of them boilerplate) (survey:skills-schemas §3). Because skills are injected wholesale, this identical prose is paid for once per section run — and up to 3× per section across repair attempts — across all 6 sections that have tools. That is the same token, billed many times, teaching nothing section-specific.
