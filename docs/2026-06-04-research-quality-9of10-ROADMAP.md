...
**2026-06-04 Codex evidence:** T6 code gate green. `loadLabSkill` now injects one shared `GTM Strategic Standard` preamble ahead of every loaded skill; six core section schemas now require common `strategicInsight` plus section-specific strategic judgment fields (`categoryPowerBet`, attack/concede + incumbent blind spot, Four Forces balance verdict, sequenced moves, falsifiability, and offer binding constraint). Validators reject broad generic filler, duplicate strategic fills, and restatements of `verdict` / `statusSummary`; VoC gap commits can still persist honest `strategicInsight` and `fourForcesBalanceVerdict` instead of terminal failure. Renderers display the new strategic fields while preserving legacy artifacts. Proof: targeted T6 Vitest 16 files / 140 tests passed; `pnpm exec tsc --noEmit` 0; `pnpm run test:run` 177 files / 1507 tests passed / 1 skipped; `pnpm run build` clean; `pnpm run lint` 0 errors / 32 existing warnings; QA re-review GO after the prior generic/repeated-fill validator blocker. Live section gate still pending.
