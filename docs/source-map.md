...
- **Agency Intelligence Console (internal, first execution slice):** fuses SaaSLaunch tracker truth (landing analytics tables: `agency_clients`, `agency_client_sites`, `landing_events`, `landing_event_rejections`) with SaaSLaunch corpus truth synced from the on-disk corpus repo. Surface: `src/app/internal/agency/page.tsx` (portfolio overview) + `src/app/internal/agency/[slug]/page.tsx` (per-client: tracker truth table, corpus stat blocks + freshness, live evidence-backed client-health insight, persisted insight history). Logic: `src/lib/agency-intelligence/` — `contracts.ts` (Zod schemas for Evidence + snapshot/current/refresh-run/insight rows), `insights/client-health.ts` (`computeClientHealth` — deterministic, no LLM, honest about absent events), `loaders.ts` (Clerk-aware Supabase reads, every query wrapped so a not-yet-provisioned `sl_*` table surfaces as a verified-absent error string). Sync script: `scripts/zz-sync-saaslaunch-corpus.mjs` (`npm run agency:sync-corpus`, `--dry-run`/`--rebuild`, manifest-hash guarded, append-only snapshots + upserted current view, failed-run audit row). Probe: `scripts/zz-probe-agency-state.mjs` (`npm run agency:probe`, read-only). Schema: `supabase/migrations/20260618_agency_intelligence.sql` (`sl_refresh_runs`, `sl_corpus_snapshots`, `sl_corpus_client_snapshots`, `sl_corpus_clients_current`, `sl_insights`; RLS = internal users select, service_role write). Plan: `docs/plans/2026-06-18-agency-intelligence-console-plan.md`. The landing-analytics tables themselves come from `supabase/migrations/20260611_saaslaunch_landing_analytics.sql` (cherry-picked from `codex/saaslaunch-analytics-prod-wire`).

- **Account-Health Cockpit (Agency Intelligence slice 2 — Fathom verbal-risk signals):** adds verified verbal-risk signals from Fathom call transcripts to the cockpit's true-risk tiering. Schema: `supabase/migrations/20260619_account_health_cockpit_fathom.sql` — `sl_fathom_transcripts` (442 raw recordings, idempotent by `raw_sha256`, `client_slug` nullable = unattributed kept-but-excluded) + `sl_fathom_signals` (gated extracted signals, attributed only; CHECK enums `signal_type ∈ {churn_escalation,going_dark,payment_risk,verbal_promise,upsell_intent}`, `severity ∈ {low,medium,high}`, `unique(recording_id,signal_type,quote_sha256)`); also widens `sl_refresh_runs.run_kind` with `fathom_sync`/`fathom_extract`. Engine: `src/lib/agency-intelligence/insights/account-health.ts` (`computeAccountHealth` — pure, no LLM; tier = max driver severity; CRITICAL only from verified verbal Fathom signals + hard stalled-launch; corpus `risk.signals[]` is a calibrated WARNING, not critical; **high-severity signals from `call_type='sales'` are capped to medium for tiering** — a prospect objection is not client churn). Anti-fabrication gate: `src/lib/agency-intelligence/fathom/gate.ts` + `scripts/_fathom-gate.mjs` (verbatim single-turn substring match, ≥12 chars; the `.mjs` mirror is pinned to the TS by `fathom/__tests__/taxonomy-drift.test.ts`). Pipeline (deterministic / offline, NO metered API): `scripts/zz-sync-fathom.mjs` (`npm run agency:sync-fathom`; `--dry-run`/`--force`/`--list-attributed [--client=]`) raw sync → `/agency-extract-signals` (LOCAL agents stage flat-array JSON per client in `tmp/fathom-extract/`) → `scripts/zz-upload-fathom-signals.mjs` (`npm run agency:upload-signals`; re-gates every quote at the DB boundary, insert-first upsert + delete-stale). Loaders join `call_type` from `sl_fathom_transcripts` into `FathomSignalLite`. Skills: `.claude/commands/agency-{sync-corpus,sync-fathom,extract-signals,refresh,sync-meta}.md`. Architecture: `docs/agency-sync-architecture.md`; spec: `docs/superpowers/specs/2026-06-18-account-health-cockpit-design.md`.
