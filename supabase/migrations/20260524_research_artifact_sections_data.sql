-- Add durable typed artifact storage to normalized section rows.
--
-- Runners now return ResearchResult.data as the full typed positioning
-- artifact. Store that payload directly on research_artifact_sections so the
-- normalized table, not journey_sessions.research_results, is the durable
-- source of truth for typed section data.
