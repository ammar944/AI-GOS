/**
 * gate.ts — Phase B deterministic provenance DETECTOR (pure, script-side).
 *
 * Catches the GLM fabrication patterns the Phase A cross-subject verdict
 * identified as the wall:
 ...
 *   3. invented_bidder / invented_volume_cpc — names an advertiser, or cites vol/CPC, no tool supports
 ...
 *
 * NO imports from src/lib — this is a throwaway zz-* script-side tool that runs
 * over the frozen tmp/zz-agentic-glm artifacts. See
 * docs/plans/2026-06-23-phase-B-provenance-gate-spec.md §1 + §5.
 */
...
export type Severity = "laundered" | "invented";
...
export interface DetectArgs {
  ...
  subject: string;
  /**
   * Raw JSON strings of SIBLING-section transcripts (same subject). Only used for
  ...
  /**
   * FIX 4 — raw text of the OTHER 6 sibling section bodies (same subject). Only used for
...
/** Extract URLs from the raw (un-normalized) transcript JSON string. */
...
interface GroundTruth {
  ...
  // FIX 4 — grounding corpus for the URL/quote checks ONLY. Equals transcriptText/Urls PLUS
  // sibling section BODIES (paidmedia carry-forward). Kept separate so invented_*/arithmetic
  // never read sibling-body prose (a section must not launder another's fabrication).
...
function buildGroundTruth(
  ...
  for (const sib of siblingTranscripts) parts.push(sib);
  const transcriptUrls = new Set<string>();
  for (const src of [rawTranscript, ...siblingTranscripts]) {
    for (const u of extractTranscriptUrls(src)) transcriptUrls.add(u);
  }
  const transcriptText = normalizeText(parts.join("  "));

  // FIX 4 — the URL/quote grounding corpus additionally folds sibling section BODIES
...
function urlGrounded(citedUrl: string, gt: GroundTruth): boolean {
  ...
  // FIX 4 — grounding corpus = transcript (+ sibling bodies for paidmedia carry-forward).
 ...
 */
function detectInventedBidder(body: string, gt: GroundTruth): Violation[] {
  const out: Violation[] = [];
...
function detectInventedVolumeCpc(body: string, gt: GroundTruth): Violation[] {
  const out: Violation[] = [];
...
function detectArithmetic(body: string): Violation[] {
  const out: Violation[] = [];
  ...
  // FIX 3 — seat-math. Two forms, both checked PER LINE (a line is the natural unit for one
...
function detectSeatMath(body: string): Violation[] {
  const out: Violation[] = [];
  const seen = new Set<string>();
...
function arithViolation(span: string, reason: string): Violation {
  return {
...
function detectInventedCustomer(body: string, gt: GroundTruth): Violation[] {
  const out: Violation[] = [];
  const seen = new Set<string>();
  const sentences = body.split(/(?<=[.!?])\s+|\n+/);
  for (const s of sentences) {
...
// Orchestration
// ---------------------------------------------------------------------------
export function detect(args: DetectArgs): DetectResult {
  const {
    bodyPath,
    transcriptPath,
    section,
  ...
  const gt = buildGroundTruth(records, rawTranscript, siblingTranscripts, siblingBodies);
  ...
  // Check 3 — invented customer (FIX 1) + invented bidder + invented volume/cpc
...
export function ceilingFor(violations: Violation[]): number {
