/**
 * gate.test.ts — TDD unit tests pinning the MECHANICS of each of the 5
 * deterministic provenance checks with small INLINE fixtures.
 *
 * These do NOT test against the real oracle cells (the verify phase does that).
 * Each test fabricates a tiny transcript+body pair that isolates one check so a
 * regression in any single mechanic surfaces here.
 *
...
import { detect, type TranscriptRecord } from "../gate";

// ---------------------------------------------------------------------------
// Fixture helper: write a body + transcript pair to a temp dir, run detect().
// ---------------------------------------------------------------------------
let DIR: string;
beforeAll(() => {
  DIR = mkdtempSync(join(tmpdir(), "prov-gate-"));
});
afterAll(() => {
...
function run(opts: {
  ...
  const id = `c${seq++}`;
  const bodyPath = join(DIR, `${id}-body.md`);
  const transcriptPath = join(DIR, `${id}-transcript.json`);
  writeFileSync(bodyPath, opts.body);
  writeFileSync(transcriptPath, JSON.stringify(opts.transcript, null, 2));
  return detect({
    bodyPath,
    transcriptPath,
...
describe("FIX 4 — paidmedia sibling-body grounding", () => {
  ...
  it("clears a paidmedia URL carried from a sibling section body", () => {
    ...
    const bodyPath = join(DIR, `${id}-body.md`);
    const transcriptPath = join(DIR, `${id}-transcript.json`);
    writeFileSync(bodyPath, "See the migration guide ([guide](https://acme.com/help/migrate)).");
    writeFileSync(
      transcriptPath,
      JSON.stringify([
        rec({
  ...
  it("still flags an invented_bidder on paidmedia even if a sibling body mentions the name (invented stays transcript-based)", () => {
    ...
    const bodyPath = join(DIR, `${id}-body.md`);
    const transcriptPath = join(DIR, `${id}-transcript.json`);
    writeFileSync(bodyPath, "Salesforce will outbid us on these CRM terms.");
    writeFileSync(
      transcriptPath,
      JSON.stringify([
        rec({
...
describe("detect() contract", () => {
  it("returns violations[] with {check,severity,ceiling,span,reason} and a stats object", () => {
    const out = run({
      section: "competitor",
  ...
  it("a clean grounded body produces zero violations", () => {
    const out = run({
      section: "demand",
      body:
