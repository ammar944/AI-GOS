---
description: Invoke the research-keywords skill
argument-hint: <input-spec>
---

# /research-keywords

Keyword and intent research — identifies high-intent search queries, content gaps, and related keywords. No fabricated volumes; every number sourced.

## Arguments

```
/research-keywords <input-spec>
```

Replace `<input-spec>` with:

- A URL (e.g. `https://airtable.com/`) — the skill resolves identity and proceeds
- Or a path to a sealed JSON payload matching `skills/research-keywords/references/input-schema.ts`

## Workflow

When invoked, Claude should:

1. Read `skills/research-keywords/SKILL.md` for the full behavior spec
2. Read `skills/research-keywords/references/collector.md` for the main agent prompt
3. Collect via native tools, fan out to subagents as specified
4. Run the deterministic tail: `cd skills/research-keywords && npm run orchestrate <run_id>`
5. Surface the HTML report path back to the user

## Status

Scaffolded 2026-04-24. Underlying skill is a stub — behavior must be implemented before this command produces real output.
