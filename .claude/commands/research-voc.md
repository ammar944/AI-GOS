---
description: Invoke the research-voc skill
...
```
/research-voc <input-spec>
```

Replace `<input-spec>` with:

- A URL (e.g. `https://airtable.com/`) — the skill resolves identity and proceeds
- Or a path to a sealed JSON payload matching `skills/research-voc/references/input-schema.ts`

## Workflow

When invoked, Claude should:

1. Read `skills/research-voc/SKILL.md` for the full behavior spec
2. Read `skills/research-voc/references/collector.md` for the main agent prompt
3. Collect via native tools, fan out to subagents as specified
4. Run the deterministic tail: `cd skills/research-voc && npm run orchestrate <run_id>`
