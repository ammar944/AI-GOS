...
- When adding an internal server-to-server route (x-internal-key auth, e.g. review-section/executive-brief kickoffs), ALWAYS add it to `isPublicRoute` in `src/middleware.ts` — Clerk 404s it otherwise, and because kickoff callers are best-effort the feature dies SILENTLY (W3 executive brief never wrote until 2026-06-11; the 404 only shows in dev-server logs). Grep test: every route checking `x-internal-key` must appear in middleware.ts.
- When a lab section passes solo but errors/starves under full fan-out (demand-intent deadline `remaining budget < fallback floor 260s`, VoC 0-quote retrieval), the cause is 6-way parallel contention, not the section — a solo production-path rerun (`/api/research-v2/rerun-section`) completes in ~2-3 min. Live-proven on run 8081e646 (2026-06-11).
...
## Skill exemplars (provenance)
- When a SKILL.md worked exemplar contains concrete domain content (named mechanisms like "OCR receipt matching", questions like "soc 2 type 2 cost", numbers), the model WILL copy it into unrelated subjects' deployable output even with maximal inline anti-copy warnings ("fictional account, automatic FAIL" was live and leaked anyway — run 8081e646). Protection must be deterministic code (`verification/provenance-gate.ts` exemplar motifs + in-artifact support check), not prompt text. When adding a new worked exemplar to a skill, add its domain motif to the gate in the same change.
